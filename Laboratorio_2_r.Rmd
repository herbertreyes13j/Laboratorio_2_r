---
title: "Laboratorio 2"
author: "Herbert Rafael Reyes Portillo"

---

#Importando paquete nycflights13
...
```{r}
flights_airports <- flights %>% inner_join(airports, by=c("origin"="faa")) %>% 
...
arrange(filter(count(flights,year,month,day),n>1000),n)
```
...
precip_delay <- flights %>% inner_join(weather, by=c("year","month","day","hour","origin")) %>%
            group_by(precip) %>%
            summarise(delay=replace_na(mean(dep_delay,na.rm=TRUE),0))
...
```{r}
flights %>% inner_join(planes, by="tailnum",suffix=c("_flights","_planes")) %>%
